(function () {
  ...
  function grade1Bank() {
    const ja = [
      ...
      ].map((x, i) => makeProblem(`g1-ja-word-${i + 1}`, 1, "国語", "input", `えカードを見て、なまえをひらがなでかきましょう。`, x[0], [x[0]], `えは「${x[0]}」です。ひらがなでていねいにかきます。`, x[2], "englishPictureCard", { icon: x[1], word: "?", label: x[2] })),
      ...[
        ["はるのカード","さくら","さ","🌸"],["じかんのカード","とけい","と","⏰"],["そらのカード","からす","か","🐦"],["くだもののカード","みかん","み","🍊"],["なつのカード","すいか","す","🍉"],["のりものカード","ひこうき","ひ","✈️"],["どうぐのカード","えんぴつ","え","✏️"],["おべんとうカード","おにぎり","お","🍙"],["いけのカード","かえる","か","🐸"],["おとカード","たいこ","た","🥁"]
      ].map((x, i) => makeProblem(`g1-ja-first-${i + 1}`, 1, "国語", "choice", `${x[0]}をよみ、さいしょのおとをえらびましょう。`, x[2], [x[2]], `カードのことばは「${x[1]}」です。さいしょに「${x[2]}」とよみます。`, `カードのことばを、ゆっくりこえに出しましょう。`, "englishPictureCard", { icon: x[3], word: x[1], label: "ことばカード" }, { choices: [x[2], kanaShift(x[2], 1), kanaShift(x[2], 2)] })),
      ...[
        ["からだカード","あし","し","🦶"],["へやカード","いす","す","🪑"],["あめの日カード","かさ","さ","☂️"],["きょうしつカード","つくえ","え","📘"],["はっぱカード","はっぱ","ぱ","🍃"],["しまカード","しま","ま","🏝️"],["そらカード","そら","ら","☀️"],["ふねカード","ふね","ね","⛵"],["あそびカード","こま","ま","🌀"],["ふゆカード","ゆき","き","❄️"]
      ].map((x, i) => makeProblem(`g1-ja-last-${i + 1}`, 1, "国語", "choice", `${x[0]}をよみ、さいごのおとをえらびましょう。`, x[2], [x[2]], `カードのことばは「${x[1]}」です。さいごのおとは「${x[2]}」です。`, `ことばをおとにわけてよみましょう。`, "englishPictureCard", { icon: x[3], word: x[1], label: "ことばカード" }, { choices: [x[2], kanaShift(x[2], 3), kanaShift(x[2], 4)] })),
      ...[
        ["おはなしの中でわたるもの","は・し", "はし"],["にわにいるいきもの","と・り", "とり"],["げんかんでぬぐもの","く・つ", "くつ"],["のどがかわいたときのむもの","み・ず", "みず"],["とびらをあけるもの","か・ぎ", "かぎ"],["いえの上にあるもの","や・ね", "やね"],["からだにきるもの","ふ・く", "ふく"],["あしが八ぽんのいきもの","た・こ", "たこ"],["あまいくだもの","な・し", "なし"],["へやにあるあかるいところ","ま・ど", "まど"]
      ].map((x, i) => makeProblem(`g1-ja-join-${i + 1}`, 1, "国語", "input", `カードのもじをつなげて、ことばをかきましょう。`, x[2], [x[2]], `カードのもじをつなげると「${x[2]}」になります。`, "あいだをあけずにかきます。", "readingCard", { title: `ことばカード ${i + 1}`, lines: [x[0], `もじ: ${x[1]}`] })),
      ...[
        ["あさの文","わたし（　）ほんをよむ。","は","する人のあとに「は」を入れると、文が自然になります。"],["ねこの文","ねこ（　）ねる。","が","ねこがどうするかを知らせる文です。"],["おやつの文","りんご（　）たべる。","を","たべるもののあとには「を」を入れます。"],["学校の文","がっこう（　）いく。","へ","行くところのあとには「へ」を入れます。"],["あそびの文","ともだち（　）あそぶ。","と","いっしょにする人のあとには「と」を入れます。"],["つくえの文","つくえ（　）うえ。","の","二つのことばをつなぐときに「の」を入れます。"],["あめの文","あめ（　）ふる。","が","何がふるのかを知らせています。"],["みずの文","みず（　）のむ。","を","のむもののあとには「を」を入れます。"],["かえる文","いえ（　）かえる。","へ","むかうところのあとには「へ」を入れます。"],["はなの文","はな（　）さく。","が","何がさくのかを知らせています。"]
      ].map((x, i) => makeProblem(`g1-ja-particle-${i + 1}`, 1, "国語", i % 2 ? "input" : "choice", `${x[0]}をよみ、（　）に入るひらがなを答えましょう。`, x[2], [x[2]], x[3], "文をこえに出してよみましょう。", "readingCard", { title: x[0], lines: [x[1]] }, i % 2 ? {} : { choices: [x[2], "を", "に"].filter((v, idx, arr) => arr.indexOf(v) === idx) }))
    ];
  ...
  function grade4Bank() {
    const ja = [
      ...[
        ["春の校庭では、季節の花が順にさきます。","季節","きせつ","春・夏・秋・冬などの時期をまとめて言う言葉です。"],
        ["最後まで努力したので、発表の声が大きくなりました。","努力","どりょく","目標に向かって力をつくすことです。"],
        ["学校の花だんで、葉の形を観察してノートに記録しました。","観察","かんさつ","よく見て調べることです。"],
        ["将来の希望を聞かれ、海の生き物を守る仕事と答えました。","希望","きぼう","こうなってほしいという願いです。"],
        ["試合の前でも、キャプテンは冷静に作戦を伝えました。","冷静","れいせい","落ち着いている様子です。"],
        ["係の仕事は、一人で抱えず協力して進めました。","協力","きょうりょく","力を合わせることです。"],
        ["集合時刻を守る約束を、班のみんなで確認しました。","約束","やくそく","守ると決めたことです。"],
        ["雨の日が三日連続したので、校庭に水たまりが残りました。","連続","れんぞく","続いていることです。"],
        ["遠足には水とうとしおりが必要です。","必要","ひつよう","なくてはならないことです。"],
        ["アンケートの結果を見て、読みたい本を選びました。","結果","けっか","ものごとの終わりに出たことです。"]
      ].map((x, i) => makeProblem(`g4-ja-read-${i + 1}`, 4, "国語", "input", `本文を読み、文中の「${x[1]}」をひらがなで答えましょう。`, x[2], [x[2]], `本文の「${x[1]}」は「${x[2]}」と読みます。${x[3]}`, "前後の文と漢字の形を合わせて考えます。", "readingCard", { title: `短い本文 ${i + 1}`, lines: [x[0]], note: `読む漢字: ${x[1]}` })),
      ...[
        ["春から夏へ、（きせつ）が少しずつ変わります。","季節","きせつ"],["目標に向かって続ける（どりょく）が実を結びました。","努力","どりょく"],["虫めがねで葉の裏を（かんさつ）しました。","観察","かんさつ"],["卒業文集に将来の（きぼう）を書きました。","希望","きぼう"],["急な知らせを聞いても、先生は（れいせい）に指示を出しました。","冷静","れいせい"],["大きな掲示物を、友だちと（きょうりょく）して作りました。","協力","きょうりょく"],["図書室では静かに読むという（やくそく）があります。","約束","やくそく"],["防災袋には、飲み水が（ひつよう）です。","必要","ひつよう"]
      ].map((x, i) => makeProblem(`g4-ja-kanji-${i + 1}`, 4, "国語", "input", `文を読み、（　）の中のひらがなを漢字で答えましょう。`, x[1], [x[1]], `文の中の「${x[2]}」は、漢字で「${x[1]}」と書きます。`, "文の中で使われている言葉を確かめます。", "readingCard", { title: `漢字カード ${i + 1}`, lines: [x[0]] })),
      ...[
        ["雨がやんだので、外で遊べる。","外で遊べる理由として合うものを選びましょう。","雨がやんだから","昼休みが短いから","ボールが新しいから","教室が静かだから","「ので」の前に、理由が書かれています。"],
        ["風は強いけれど、船は進んだ。","文の内容に合うものを選びましょう。","風は強かったが船は進んだ","風がないので船は止まった","船は風を見ていなかった","海ではなく山の話である","「けれど」は、前と後でちがう様子をつなぎます。"],
        ["朝になったら、鳥が鳴き始めた。","鳥が鳴き始めた時を選びましょう。","朝になった時","夜の終わる前","昼休みの後","夕方の放送の後","「朝になったら」が、時を知らせています。"],
        ["本を読むために、図書館へ行った。","図書館へ行った目的を選びましょう。","本を読むこと","給食を食べること","走る練習をすること","水をくむこと","「ために」の後ろに、目的に合う行動が続いています。"],
        ["暑いから、窓を開けた。","窓を開けた理由として合うものを選びましょう。","暑かったから","雨が強かったから","漢字を覚えたから","本を借りたから","「から」の前が理由です。"],
        ["練習したのに、うまくできなかった。","文の内容に合うものを選びましょう。","練習したがうまくできなかった","練習しなかったので成功した","練習前に発表が終わった","友だちが来なかった","「のに」は、予想とちがう結果をつなぎます。"],
        ["夕方になると、町の明かりがついた。","町の明かりがついた時を選びましょう。","夕方になった時","朝の会の時","雨がやんだ時","給食の前","「夕方になると」が、時を知らせています。"],
        ["発表するために、資料を集めた。","資料を集めた目的を選びましょう。","発表すること","昼寝をすること","水そうを洗うこと","遠足へ行くこと","「発表するために」が目的を示しています。"]
      ].map((x, i) => makeProblem(`g4-ja-connect-${i + 1}`, 4, "国語", "choice", x[1], x[2], [x[2]], x[6], "前の文と後ろの文のつながりを考えます。", "readingCard", { title: `文のつながり ${i + 1}`, lines: [x[0]] }, { choices: [x[2], x[3], x[4], x[5]] })),
      ...[
        ["芽が出るまで時間がかかったが、春人さんは根気強く水やりを続けた。","この文の「根気強く」に近い内容を選びましょう。","あきらめずに続ける様子","すぐにやめる様子","大きな声で読む様子","遠くへ走る様子","水やりを続けた様子から考えます。"],
        ["雨でも読めるように、係はポスターを透明なふくろに入れる工夫をした。","係がしたことに近い内容を答えましょう。","よい方法を考えた","急いで片づけた","名前を消した","声を小さくした","困りそうなことを防ぐ方法を考えています。"],
        ["ガラスの近くでは、慎重に荷物を運んだ。","この文の「慎重に」に近い内容を選びましょう。","よく注意して","ふざけながら","何も見ずに","歌いながら","ガラスの近くなので、注意して運ぶ場面です。"],
        ["森には虫や鳥が多く、音も色も豊かだった。","森の様子として合う内容を答えましょう。","いろいろなものが十分にある","何もなく静かである","同じ色しかない","すぐ終わる","虫や鳥、音や色が多いことから考えます。"],
        ["雲の動きを見て、明日の天気を予想した。","この文でしていることを選びましょう。","これからどうなるか考えること","終わったことを消すこと","同じ字を写すこと","音を合わせること","明日の天気は、これからのことです。"],
        ["発表前に、調べたことを表にして整理した。","表にした目的に近い内容を答えましょう。","分かりやすく整えること","すぐに忘れること","声を大きくすること","水を入れること","表にすると、内容を見やすくできます。"],
        ["二つの橋の長さを比較して、近い道を選んだ。","この文でしていることを選びましょう。","二つを比べること","一つをかくすこと","同じ文を読むこと","花を育てること","二つの橋の長さを見比べています。"],
        ["司会、記録、発表の役割を決めてから話し合いを始めた。","班で決めたことに近い内容を答えましょう。","それぞれが受け持つ仕事","全員で同じ紙を持つこと","時間を知らせる音","教室の場所","司会や記録は、それぞれの仕事です。"]
      ].map((x, i) => makeProblem(`g4-ja-vocab-${i + 1}`, 4, "国語", i % 2 ? "input" : "choice", x[1], x[2], [x[2]], x[6], "前後の文から、言葉の使われ方を考えます。", "readingCard", { title: `ことばの場面 ${i + 1}`, lines: [x[0]] }, i % 2 ? { longAnswer: true } : { choices: [x[2], x[3], x[4], x[5]] })),
      ...[
      ...
      ].map((x, i) => makeProblem(`g4-ja-readtext-${i + 1}`, 4, "国語", i % 2 ? "input" : "choice", `本文を読み、中心になる行動を答えましょう。`, x[1], [x[1]], x[2], "だれが、何のために行動したかに注目します。", "readingCard", { title: `読解カード ${i + 1}`, lines: splitJapaneseLine(x[0]) }, i % 2 ? { longAnswer: true } : { choices: [x[1], "関係のない遊び", "文字の大きさだけ", "天気の名前だけ"] })),
      ...[
      ...
      ].map((x, i) => makeProblem(`g4-ja-think-${i + 1}`, 4, "国語", i % 2 ? "input" : "choice", `場面を読み、人物の行動として大切なことを答えましょう。`, x[1], [x[1]], x[2], "行動と、その理由をつなげて考えます。", "readingCard", { title: `考える読解 ${i + 1}`, lines: splitJapaneseLine(x[0]) }, i % 2 ? { longAnswer: true } : { choices: [x[1], "早く終わらせたこと", "関係のないことを話したこと", "何もしなかったこと"] }))
    ];
